...
class FClassModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(FClassModel, self).__init__(opt)
    ...
    def feed_data(self, data):
        ...
        self.quality = data['quality'].to(self.device)
    ...
    def optimize_parameters(self, current_iter):
        ...
        self.output = hyper_out

        l_total = 0
        loss_dict = OrderedDict()
        # cross entropy loss
        if self.cri_ce:
            l_pix = self.cri_ce(self.output, self.quality)
            l_total += l_pix
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            metric_data['gt'].append(self.quality.cpu().numpy())
