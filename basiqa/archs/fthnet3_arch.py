# --------------------------------------------------------
# Swin Transformer
# Licensed under The MIT License [see LICENSE-swin-transformer for details]
# --------------------------------------------------------
import torch
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        # print('processing input of ',x.shape)
...
class SwinBackbone(nn.Module):
    r""" Hyper Net backbone built with feature extractor Swin Transformer
    ...
    def __init__(self, img_size=384, patch_size=4, in_chans=3, ffa_out_ch=32, tn_in_ch=384,
        ...
        self.feature_ch = int(embed_dim * 2 ** (self.num_layers - 1))
        self.mlp_ratio = mlp_ratio
        self.window_size=window_size
        # self.bb_out_ch=out_dims
        ...
        self.fcl_merge = nn.ModuleList()
        for ind in range(4):
          sub_merge=nn.Sequential(
            nn.Conv2d(int(embed_dim * 2 ** ind),int(embed_dim * 2 ** (ind-3)),stride=1,kernel_size=1,padding=0,bias=False),
            nn.AvgPool2d(stride=window_size,kernel_size=window_size),
          )
          self.fcl_merge.append(sub_merge)

        self.fcl_fc = nn.ModuleList()
        for ind in range(3):
            # 首先进行一次降维，然后进行平均池化例如[ 16, 145, 192]会变为 [ 16, 1, 192]
            # 然后在FC层中，192,384,768变为目标网络所需要的通道数量(32)
            # embed_dim * 2 ** ind-3 is the channel, (8/2**ind)**2 is the number of flattened patch
            sub_fc = nn.Linear(int(embed_dim * 2 ** (ind-3) * (8/2**ind)**2), ffa_out_ch)
            self.fcl_fc.append(sub_fc)
        # 将剩余特征全部转化为收集为feature
        sub_fc = nn.Linear(int(embed_dim* (8/2**3)**2), tn_in_ch - ffa_out_ch * 3)  # 总的通道数对齐到target network input channel
        self.fcl_fc.append(sub_fc)
        # 将输出变为可以适应下一步输出的12**2维
        self.norm = norm_layer(self.feature_ch)
    ...
    def forward(self, x):
        ...
        for ind in range(4):
            x = self.layers[ind](x)
            fcl_results.append(self.fcl_fc[ind](self.fcl_merge[ind](x.view(x.size(0),-1, self.window_size*2**(3-ind), self.window_size*2**(3-ind))).view(x.size(0), -1)))
...
class FTHNet3(nn.Module):
    """Hyper network for learning perceptual rules. Based on the MSG transformer backbone with structure modified."""

    def __init__(self, ffa_out_ch=32, tn_in_ch=384, hyper_in_ch=384,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                 window_size=12, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, ):
        super(FTHNet3, self).__init__()
        self.target_input_channel = tn_in_ch
        self.num_layers=len(depths)
        self.hyper_input_channel = hyper_in_ch
        self.feature_size = window_size
        self.feature_ch = int(embed_dim * 2 ** (self.num_layers - 1))

        # 基础骨架使用上方的convNext，所有参数可以调节
        self.bbone = SwinBackbone(ffa_out_ch=ffa_out_ch, tn_in_ch=tn_in_ch,
        ...
        sub_1x1 = nn.Sequential(nn.Conv2d(self.feature_ch, hyper_in_ch, stride=1, kernel_size=1, padding=0), nn.GELU(), )
        ...
        for ind in range(4):
            sub_weight = nn.Conv2d(int(hyper_in_ch / 2 ** ind), int(tn_in_ch ** 2 / 2 / self.feature_size ** 2 / (4 ** ind)),
        ...
        sub_weight = nn.Linear(int(hyper_in_ch / 2 ** 4 * self.feature_size ** 2), int(tn_in_ch / 2 ** 4))
        sub_bias = nn.Linear(int(hyper_in_ch / 2 ** 4 * self.feature_size ** 2), 1)
        self.weight_conv.append(sub_weight)
        self.bias_conv.append(sub_bias)
        self.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        hyper_in = bb_out['hyper_in_feat'].view(-1, self.feature_ch, self.feature_size, self.feature_size)
        # 生成1-4层Fc的参数
        tn_para_w = []
        tn_para_b = []
        for ind in range(4):
        ...
        tn_para_w.append(self.weight_conv[4](hyper_in).view(-1, 1, int(self.target_input_channel / 2 ** 4), 1, 1))
        tn_para_b.append(self.bias_conv[4](hyper_in).view(-1, 1))
